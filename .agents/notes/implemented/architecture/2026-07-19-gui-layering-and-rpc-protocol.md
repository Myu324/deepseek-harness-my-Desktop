...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Web (browser access) only; zero workspace dependencies (the registry arrives by structural injection); the desktop shell loads this whole surface over the same loopback carrier rather than an IPC bridge ([Windows desktop shell note](2026-08-14-windows-desktop-shell.md)) |
...
| IPC bridge subclass (hypothetical example — the desktop shell does not use it yet) | an Electron shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
